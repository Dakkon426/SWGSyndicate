...
void EntertainingSessionImplementation::activateEntertainerBuff(CreatureObject* creature, int performanceType) {
	...
	try {
		...
		// Apply both types of buffs regardless of performance type
		// Music buffs (focus and willpower)
		...
		if (oldFocusBuff == nullptr || oldFocusBuff->getBuffStrength() <= buffStrength) {
		...
		if (oldMindBuff == nullptr || oldMindBuff->getBuffStrength() <= buffStrength) {
